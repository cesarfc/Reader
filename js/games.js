...
(function () {
  ...
  const flashGame = {
    ...
    start(container, ctx) {
      ...
      function show() {
        ...
        shell.nowDot(i);
        shell.area.innerHTML = `
          <div class="speedtop"><span class="clock">0s</span></div>
    ...
    start(container, ctx) {
      ...
      function lightning() {
        ...
        function show() {
          ...
          shell.area.querySelector('[data-act="got"]').addEventListener('click', () => {
            outcomes.push({ k: 's:' + deck[i], ok: true });
            score++; A.sfx.tick(); i = (i + 1) % deck.length; show();
          });
  ...
  /* =========================================================
     ...
     A library of decodable books per grade. The reader reads out
     loud; any word can be tapped for help, and a whole page can be
     read aloud. Fewer helps = more stars.
     ========================================================= */
  const booksGame = {
    title: 'Story Books',
    icon: '📚',
    ...
    start(container, ctx) {
      ...
      function reader(book) {
        ...
        function show() {
          ...
          container.innerHTML = `
            <header class="gamebar">
              <button class="iconbtn" data-act="back" aria-label="Back to bookshelf">←</button>
          ...
          container.querySelector('[data-act="back"]').addEventListener('click', library);
          ...
          if (!prev.disabled) prev.addEventListener('click', () => { page--; A.sfx.tick(); show(); });
