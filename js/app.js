  ...
  function journeyCardHtml(p) {
    ...
    return `
      <button class="journeycard ${ready ? 'ready' : ''}" data-act="journey">
        ...
        <div class="timebar jbar"><i style="width:${pct}%"></i></div>
        ${ready ? '<div class="jready">🎓 READY TO GRADUATE — TAP!</div>'
                : '<div class="jhint">Master words, sight words, books & speed to fill the rocket!</div>'}
      </button>`;
  }
        ...
        ${p.quests.items.map(it => {
          ...
          return `
            <div class="questrow ${it.done ? 'done' : ''}">
              ...
              <span class="qname">${def.name}</span>
              <span class="qprog">${it.done ? `+${def.reward} 🪙` : (def.target > 1 ? `${prog}/${def.target}` : '·')}</span>
            </div>`;
        }).join('')}
      </div>`;
  }
    ...
    app.innerHTML = `
      <section class="screen">
        ...
        <div class="herocard">
          ...
          <div class="heromid">
            <div class="herostats">⚔️ ${S.attack(p)} attack · ❤️ ${S.hearts(p)} hearts${S.coinBoost(p) ? ` · 🪙 +${S.coinBoost(p)}%` : ''}${S.streakBoost(p) ? ` · 🔥 +${S.streakBoost(p)}%` : ''}</div>
            <div class="herogear">${gearChips}</div>
          </div>
          <div class="herobtns">
            <button class="btn shopgo" data-act="shop">🛒<br>Shop</button>
            <button class="btn ghost shopgo" data-act="badges">🏅<br>Badges</button>
          </div>
        </div>
    ...
    app.innerHTML = `
      <section class="screen">
        <header class="gamebar">
          <button class="iconbtn" data-act="back" aria-label="Back">←</button>
          <div class="gametitle">🏅 Badges</div>
  ...
  function renderResults(gameId, r, meta) {
    ...
    app.innerHTML = `
      <section class="screen results">
        ...
        ${meta.levelUp ? `<p class="levelup">⬆️ LEVEL ${meta.levelUp} — ${P.titleOf(meta.levelUp)}!</p>` : ''}
        ${!meta.gemsEarned && r.stars < 3 ? '<p class="gemhint">💎 Get 3 stars to win a gem!</p>' : ''}
        ${meta.newlyMastered && meta.newlyMastered.length ? `
          <div class="masteredrow">⭐ Mastered: ${meta.newlyMastered.map(w => `<span class="masterychip">${esc(w)}</span>`).join('')}</div>` : ''}
        ${(meta.questsDone || []).map(q => `<p class="questdone">🎯 ${q.e} ${q.name}${q.reward ? ` +${q.reward} 🪙` : ''}${q.gems ? ` +${q.gems} 💎` : ''}</p>`).join('')}
    ...
    setTimeout(() => A.sfx.coins(), 500);
    if (meta.gemsEarned) setTimeout(() => A.sfx.gem(), 850);
    if (meta.levelUp) setTimeout(() => { A.sfx.victory(); RR.confetti.burst(80); }, 1100);
